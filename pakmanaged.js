...
// pakmanager:domready/ender-bridge
(function (context) {
...
(function (context) {
  ...
  /**
    ...
    !function (name, context, definition) {
      ...
      Youtube.setContainer = function(ele) {
        Youtube.container = jQuery(ele);
      };
    
      Youtube.loadVideo = function(youtubeIdOrLink, userOptions) {
        ...
        var youtubeId = Youtube.extractYoutubeId(youtubeIdOrLink);
        if (!youtubeId) {
        ...
        if (Youtube.container.find('#ytapiplayer').length == 0) {
          Youtube.container.append(jQuery('<div id="ytapiplayer"/>'));
        }
        ...
        swfobject.embedSWF(youtubeEmbedLink, "ytapiplayer",
          jQuery(window).width(), jQuery(window).height(), "8", null, null,
          { allowScriptAccess: "always" }, { id: "ytplayer" });
    
        Youtube.currentYoutubeId = youtubeId;
      };
    
      Youtube.unloadVideo = function() {
        if (Youtube.player) {
          swfobject.removeSWF(Youtube.player.id);
          Youtube.player = null;
        }
      };
    
      // Reloads a currently playing video and resumes at the same place.
      Youtube.reloadVideo = function() {
        if (!Youtube.currentYoutubeId) { return; }
        var resumeTime = Youtube.player.getCurrentTime();
        Youtube.loadVideo(Youtube.currentYoutubeId, resumeTime);
      };
    
      // Stolen from underback.js
      Youtube.debounce = function(func, wait, immediate) {
        var timeout, result;
        return function() {
      ...
      Youtube.resizeVideo = Youtube.debounce(function(e) {
        if (Youtube.player) {
          jQuery(Youtube.player).width(jQuery(window).width()).height(jQuery(window).height());
        }
      }, 500);
    
      // Accepts a link or id (youtube format is 11 alphanumeric characters), parses out the
      // youtube id and returns it.
      Youtube.extractYoutubeId = function(idOrLink) {
      ...
      Youtube.onYouTubePlayerReady = function(playerId) {
        ...
        // Youtube.player.addEventListener('onStateChange', 'stateChange');
      };
  ...
  provide("youtube-mayhem", module.exports);
}(global));